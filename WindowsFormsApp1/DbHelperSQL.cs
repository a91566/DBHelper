...
namespace ZR.Base
{
    ...
    public abstract  class DbHelperSQL
    {
        ...
        public static string connectionString = @"server=192.168.123.187\SQLSERVER2005;database=ZrTBM;uid=sa;pwd=123456";
        ...
        /// </summary>
        /// <param name="SQLStringList">多条SQL语句</param>		
        public static int ExecuteSqlTran(List<String> SQLStringList,List<List<SqlParameter>> SQLParamList)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                ...
                //{
                    ...
                    for (int n = 0; n < SQLStringList.Count; n++)
                    {
                        ...
                        SqlParameter[] cmdParms = SQLParamList[n].ToArray();
                        PrepareCommand(cmd, conn, tx, cmdText, cmdParms);
                        int val = cmd.ExecuteNonQuery();
                        count += val;
                        cmd.Parameters.Clear();
        ...
        /// </summary>
        /// <param name="SQLStringList">多条SQL语句</param>		
        public static int ExecuteDataBaseSqlTran(String sqlCon,List<String> SQLStringList)
        {
        ...
        public static DataTable QueryTab(string sql) 
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                //{
                //  // ErrorLog.WriteError.Recode(ex);
                //  //  SystemLogHelper.ofExSQL(sql, SystemLogHelper.LogLevel.DEBUG.ToString());
                //    //throw new Exception(ex.Message);
                //}
                return dt;
            }
        ...
        public static DataTable QueryTab(string sql, params SqlParameter[] pms)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand();
                PrepareCommand(cmd, connection, null, sql, pms);
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    DataTable dt = new DataTable();
                    try
